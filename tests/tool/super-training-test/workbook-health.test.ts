...
describe("workbook health", () => {
  ...
  beforeAll(() => {
    const context = loadBrowserScripts([
      ...
      "tool/app/super-training-test/scripts/workbook-health.js"
    ], {
      XLSX
    });
    ...
    WorkbookHealth = superTraining.WorkbookHealth;
  });
  ...
  it("reports current workbook structure and data health without duplicating business rules", () => {
    ...
    expect(messages.some((message: string) => message.includes("工作表“新雇员培训”当前不参与超级培训监控。"))).toBe(true);
  });
});
