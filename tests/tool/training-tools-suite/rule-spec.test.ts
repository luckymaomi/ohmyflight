...
describe("training-tools-suite rule spec", () => {
  let context: ReturnType<typeof loadBrowserScripts>;
  let TrainingTools: any;

  beforeAll(() => {
    context = loadBrowserScripts([
      "tool/app/training-tools-suite/scripts/core.js",
      "tool/app/training-tools-suite/scripts/rule-engine.js"
    ]);
    TrainingTools = context.TrainingTools;
  });

  it("matches the published training rule matrix, aliases, renewal examples, and schedule examples", () => {
    const ruleMap = TrainingTools.getDefaultRuleMap();
    ...
    TRAINING_RULE_DEFINITIONS.forEach((definition) => {
      ...
      definition.aliases.forEach((alias) => {
        expect(TrainingTools.normalizeProjectName(alias)).toBe(definition.project);
      });
    });

    TRAINING_RENEWAL_CASES.forEach((testCase) => {
      const rule = ruleMap.get(testCase.project);
      const oldExpiry = TrainingTools.parseDate(testCase.oldExpiry);
      const trainingDate = TrainingTools.parseDate(testCase.trainingDate);
      const computed = TrainingTools.computeExpiry(rule, trainingDate, oldExpiry);
      const judgement = TrainingTools.classifyUpdateJudgement(rule, trainingDate, oldExpiry);
      const windowInfo = TrainingTools.getWindowInfo(rule, oldExpiry);

      expect(
        TrainingTools.formatDate(computed.newExpiry),
      ...
      if (testCase.expectedWindowStart && testCase.expectedWindowEnd) {
        ...
        expect(TrainingTools.formatDate(windowInfo.windowStart)).toBe(testCase.expectedWindowStart);
        expect(TrainingTools.formatDate(windowInfo.windowEnd)).toBe(testCase.expectedWindowEnd);
      } else {
    ...
    TRAINING_SCHEDULE_CASES.forEach((testCase) => {
      const rule = ruleMap.get(testCase.project);
      const result = TrainingTools.classifyScheduleStageStatus(
        rule,
        TrainingTools.parseDate(testCase.stageStart),
        TrainingTools.parseDate(testCase.stageEnd),
        TrainingTools.parseDate(testCase.oldExpiry)
      );
