...
- 结果 Excel：批量粘贴和 Excel 导入都会新建结果 Excel，每处理一条实时写入一条结果
- 失败报告：记录并输出失败的条目，便于复制重跑
...
3. 选择批量/手动模式
4. 粘贴数据 → 确认 → 新建结果 Excel → 自动填表 → 提交 → 检查冲突 → 实时写入结果
5. 无冲突继续下一条，有冲突或错误自动跳过；成功备注为空，失败或冲突备注写入原因
...
- 全局无超时限制，等待元素出现后再操作
- Excel导入只读取原文件，不会回写原文件；处理结果写入新建的结果 Excel
